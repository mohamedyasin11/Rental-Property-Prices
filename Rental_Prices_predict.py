...
latitude = st.sidebar.number_input(label='enter the latitude: ',format="%.2f")

longitude = st.sidebar.number_input('enter the longitude: ',format="%.2f")
...
property_size =  st.sidebar.number_input('enter the property_size: ',format="%.2f")

property_age =  st.sidebar.number_input('enter the property_age: ',format="%.2f")
...
if None not in data and st.button('predict'):
    ...
    model = pickle.load(open('Rental_Price_RF.pkl','rb'))
